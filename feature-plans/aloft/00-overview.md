...
| 53 | Focus circle | The study tab is one shape: a ring around a round 3D room she walks in, then the number, then the button | The owner wanted the model inside the circle, bigger, moving around and working, in the spirit of Forest. The room is a platform diorama in three.js (curved back wall, desk, shelf, milestone props as boxes) instead of the SVG layers. She has a plan per mood: wandering between spots on the idle screen, seated and writing while studying with a walk to the shelf every couple of minutes, standing and waiting when the tab is away, leaning back on a break. Poses blend on the bones; the walk is the rig's one clip played in place while the group moves. The idle stack is the headline, the circle, the aim chip, the length as big digits, the length chips, then Start; the weekly card, today's lanterns, and the next milestones moved under the button. On phones the action row is sticky above the tab bar (critique P1). | 2026-09-09 |
